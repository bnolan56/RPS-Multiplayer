...
$(document).on('ready', function() {
  ...
  // variables for Firebase
	var database = new Firebase("https://rps-game-bebb1.firebaseio.com/");
	var presenceRef = new Firebase("https://rps-game-bebb1.firebaseio.com/.info/connected");
	var playersRef = new Firebase("https://rps-game-bebb1.firebaseio.com/playersRef");
	var turnRef = new Firebase("https://rps-game-bebb1.firebaseio.com/turn");
  ...
  // removes turn and ends game when player disconnects
	...
	var game = {
      ...
      // looks for two or more players
        ...
        if (turnVal !== null && player == undefined) {
			...
			// adds name and sets player on click
			...
			playersRef.on('child_added', function(childSnapshot) {
				...
				// fetches player number
				var key = childSnapshot.key();

				// fetches player names
				name[key] = childSnapshot.val().name;

				// removes waiting for player screen and adds player name
			...
			playersRef.on('child_removed', function(childSnapshot) {
				// finds player that was removed
				var key = childSnapshot.key();

				// shows 'player has disconnected' on chat
		...
		addPlayer: function(count) {
			...
			// Show greeting
			var $hi = $('<h6>').text('KONICHIWA ' + playerName + '! YOU ARE PLAYER ' + player);
			var $h6 = $('<h6>');
			greeting.append($hi).append($h6);
...
var chat = {
		...
		showMessage: function(playerName, message) {
			...
			if (name[1] == playerName) {
				$p.addClass('light-blue accent-3');
			// If player 2 -> red text
			} else if (name[2] == playerName) {
				$p.addClass('red accent-3');
			}
			// Append message
			if ($p.text() !== '') {
				$('#messages').append($p);
